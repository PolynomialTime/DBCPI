...
class CoopHunt(gym.Env):
    """
    Health for each agent
    """
    def __init__(self, n_agents=4, mod_rew=False,normalize_obs=True, dif_ability=True) -> None:
        super().__init__()
        self.punishment_reward = 1.5
        self.all_states = None
        self.env_name = 'Hunt'
        self.dif_ability = dif_ability
        ...
        for i in visit_sequence:
            if self.s[i] == 0:  # In
                if act[i] == 0:
                    ...
                    if self.dif_ability and i==0:
                        reward_val += self.hunt_reward*3
                        new_state[i] = 1
                    else:
                        reward_val += self.hunt_reward
                        new_state[i] = 1
            elif self.s[i] == 1:  # Out
                if act[i] == 0:
                    new_state[i] = 0
                    ...
                    reward_val -= self.hunt_reward
        reward = np.repeat(reward_val,repeats=self.num_agents)
        self.s = new_state
