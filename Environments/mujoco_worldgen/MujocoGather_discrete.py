...
class MujocoGather():
    def __init__(self, seed=None, modify_reward=False, render=False) -> None:
        ...
        self.env = env
        self.is_obs_format = True
        self.num_agents = 2
        self.observation_space = self.num_agents*2 + 6 + 3  # agent_position + food_position + food_health
        self.ori_idx = None
        self.alter_idx = None
    ...
    def step(self, action, render=False):
        ...
        food_health = food_health.reshape(3)
    ...
    def obs_to_discrete(self, obs):
        if self.is_obs_format:
            obs = obs * 10
        agent_pos = obs[:2*self.num_agents]
        food_pos = obs[2*self.num_agents:-3]
        agent_pos = np.ceil(agent_pos/2)
        food_pos = np.ceil(food_pos/2)
        discrete_obs = np.concatenate(agent_pos,food_pos,obs[-3:])
        return discrete_obs
