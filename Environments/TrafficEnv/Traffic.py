...
class TrafficEnv(gym.core.Env):
    ...
    def __init__(self,
        ...
        # states_reload = states_reload[int(len(states_reload))]
        ...
        self.reload_index_to_coor = []
        self.coor_reloads_obs = []

        for trans in self.transitions:
            if trans['reload']:
                x, y = trans['coor']
                self.reload_index_to_coor.append((x, y))
                self.coor_reloads_obs.append(x)
                self.coor_reloads_obs.append(y)
        self.num_reloads = len(self.reload_index_to_coor)
        
        self.target_index_to_coor = []
        self.s_of_target = []
        self.coor_targets_obs = []
        i = 0
        for trans in self.transitions:
            if trans['target']:
                x, y = trans['coor']
                self.target_index_to_coor.append((x,y))
                self.s_of_target.append(i)
                self.coor_targets_obs.append(x)
                self.coor_targets_obs.append(y)
            i = i + 1
        self.num_targets = len(self.target_index_to_coor)

        self.target_visited = np.repeat(0,self.num_targets)
        ...
        self.observation_space = 2*self.num_agents + self.num_agents + 2*self.num_reloads  + 3*self.num_targets
        # x-y of agents, energy of agents, x-y of reloads, x-y of targets, if-picked of targets
        self.action_space = 2
    ...
    def reset(self):
        ...
        self.target_visited = [False for _ in range(self.num_targets)]
        ...
        obs = np.concatenate([pos_agent.flatten(),self.energy_levels,self.coor_reloads_obs,self.coor_targets_obs,self.target_visited])
        ...
        for i in self.agent_visit_seq:
            if self.alives[i]:  # Still alive
                ...
                if self.transitions[next_s]['target'] == True:  # Assign target reward
                    ind_target = self.s_of_target.index(next_s)
                    self.target_visited[ind_target] = 1
                    reward[i] += 1
    ...
    def is_state_danger(self, state):
        energy_levels = state[2*self.num_agents:3*self.num_agents]
