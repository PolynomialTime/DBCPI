...
class Lever(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.modrew:
            ...
            if self.is_state_danger(self.state):
                reward[0]+=self.punish_reward
                reward[1]+=self.punish_reward
        done = False
        ...
        for i in range(len(food_old_pos)):
            ...
            if len(agents_eat) > 0:
                ...
                reward[agent_eat] += self.positive_reward  # Assign reward
    ...
    def transition_prob(self, s_from, s_to, act):
        ...
        if np.any(agent_new_pos != agent_to_pos):  # agent impossible
            ...
            if sum(food_eaten) == 0:  # agent can't eat food
        ...
        for fp in food_old_pos:
            ...
            for i in range(self.num_agents):
                if fp == agent_new_pos[i]:
                    agent_get_food[i] = 1
            if sum(agent_get_food) != 0: # Someone got reward
                avg_reward = self.positive_reward / sum(agent_get_food)
                reward[np.where(agent_get_food==1)[0]] += avg_reward
        if self.modrew:
