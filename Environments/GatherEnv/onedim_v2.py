...
class OneDimGather(gym.Env):
    ...
    def __init__(self, read_file = True, transition_mat_path = "./onedim_tran_mat.pkl", n_agents=2, n_food = 2, modrew=False):
    ...
    def step(self, actions):
        ...
        for i in range(len(food_old_pos)):
            ...
            if len(agents_eat) > 0:
                ...
                food_old_pos[i] = np.random.choice([-1,1])  # Generate new food
        self.state = np.append(agent_new_pos, food_old_pos)
        if max(self.scores) >= 10:
            done = True
        return self.state, reward, done, {}
    def is_state_danger(self,state):
        ap = self.state[:self.num_agents]
        if abs(sum(ap)) == 2:
            return True
        else:
            return False
    def all_states(self):
        return product([-1,0,1],[-1,0,1],[-1,1],[-1,1])
    ...
    def reset(self):
        ...
        food_pos = np.random.choice([-1,1],2)
